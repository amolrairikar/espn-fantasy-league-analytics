...
resource "aws_s3_bucket" "application_bucket" {
  bucket = var.environment == "prod" ? "${data.aws_caller_identity.current.account_id}-fantasy-recap-app-database" : "${data.aws_caller_identity.current.account_id}-fantasy-recap-app-database-dev"
