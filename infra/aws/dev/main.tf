...
resource "aws_dynamodb_table" "application_table" {
  name         = "fantasy-analytics-app-db-dev"
...
resource "aws_iam_role" "lambda_role" {
  name               = "fantasy_analytics_app_lambda_role_dev"
...
data "aws_iam_policy_document" "lambda_policy" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:states:us-east-2:${data.aws_caller_identity.current.account_id}:stateMachine:league-onboarding-dev"
    ]
  }
...
resource "aws_iam_role_policy" "lambda_role_policy" {
  name   = "fantasy_analytics_app_lambda_access_policy_dev"
...
resource "aws_lambda_function" "api_lambda" {
  function_name                  = "fantasy-analytics-api-lambda-dev"
...
resource "aws_api_gateway_rest_api" "fastapi_api" {
  name        = "fantasy-analytics-app-api-dev"
...
resource "aws_api_gateway_integration_response" "proxy_options_integration_response" {
  ...
  response_parameters = {
    ...
    "method.response.header.Access-Control-Allow-Origin"      = "'https://d3ee0pfrs1t35a.cloudfront.net'"
    "method.response.header.Access-Control-Allow-Credentials" = "'true'"
  }
...
resource "aws_api_gateway_stage" "development" {
  depends_on    = [aws_cloudwatch_log_group.api_dev_log_group]
  deployment_id = aws_api_gateway_deployment.deployment.id
  rest_api_id   = aws_api_gateway_rest_api.fastapi_api.id
  stage_name    = "development"
  access_log_settings {
    destination_arn = aws_cloudwatch_log_group.api_dev_log_group.arn
    format = jsonencode({
...
resource "aws_api_gateway_usage_plan" "usage_plan" {
  name = "fantasy-analytics-app-api-usage-plan-dev"
...
resource "aws_api_gateway_deployment" "deployment" {
  ...
  # Add this trigger block to force redeployment on changes to API resources/integrations
  triggers = {
...
resource "aws_cloudwatch_log_group" "api_dev_log_group" {
  name = "api-gateway-execution-logs-${aws_api_gateway_rest_api.fastapi_api.id}/development"
  tags = {
    Project     = "fantasy-analytics-app"
    Environment = "DEV"
  }
...
resource "aws_iam_role" "api_gateway_cloudwatch_role" {
  name = "api-gateway-cloudwatch-logs-role-dev"
...
resource "aws_s3_bucket" "react_site" {
  bucket = "${data.aws_caller_identity.current.account_id}-fantasy-insights-app-react-site-dev"
  tags = {
    Project     = "fantasy-analytics-app"
    Environment = "DEV"
  }
...
resource "aws_cloudfront_origin_access_control" "oac" {
  name                              = "fantasy-insights-react-app-oac-dev"
...
resource "aws_cloudfront_distribution" "react_distribution" {
  ...
  origin {
    ...
    origin_id                = "react-app-s3-dev"
    origin_access_control_id = aws_cloudfront_origin_access_control.oac.id
  }

  default_cache_behavior {
    target_origin_id       = "react-app-s3-dev"
  ...
  tags = {
    Project     = "fantasy-analytics-app"
    Environment = "DEV"
    Name        = "fantasy-insights-app-cloudfront-distribution-dev"
  }
}
...
resource "aws_iam_role" "step_functions_role" {
  name = "league-onboarding-sfn-role-dev"
...
resource "aws_iam_role_policy" "step_functions_policy" {
  name = "league-onboarding-sfn-policy-dev"
...
resource "aws_lambda_layer_version" "shared_dependencies_layer" {
  layer_name          = "league_onboarding_shared_dependencies_dev"
...
resource "aws_lambda_function" "league_members_lambda" {
  function_name    = "fantasy-analytics-league-members-lambda-dev"
...
resource "aws_lambda_function" "league_scores_lambda" {
  function_name    = "fantasy-analytics-league-scores-lambda-dev"
...
resource "aws_lambda_function" "league_standings_lambda" {
  function_name    = "fantasy-analytics-league-standings-lambda-dev"
...
resource "aws_lambda_function" "league_weekly_standings_lambda" {
  function_name    = "fantasy-analytics-league-weekly-standings-lambda-dev"
...
resource "aws_lambda_function" "league_records_lambda" {
  function_name    = "fantasy-analytics-league-records-lambda-dev"
...
resource "aws_lambda_function" "league_draft_picks_lambda" {
  function_name    = "fantasy-analytics-league-draft-picks-lambda-dev"
